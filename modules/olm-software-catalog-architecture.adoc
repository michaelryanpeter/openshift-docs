...
where:
+
`spec.disableAllDefaultSources`:: Specifies an override that controls availability of all default catalogs that are configured by default during an {product-title} installation.
`spec.sources.disabled`:: Disables default catalogs individually by changing the `disabled` parameter value per source.
