...
`config.openshift.io/inject-trusted-cabundle`:: Specifies the label that requests the Cluster Network Operator to inject the merged bundle.
+
...
----
+
* `config` adds a configuration section if it does not exist.
+
* `selector.matchLabels.<labels_for_pods>` specifies labels to match pods that are owned by the Operator.
+
* `volumes` creates a `trusted-ca` volume.
+
* `key: ca-bundle.crt` is required as the config map key.
+
* `path: tls-ca-bundle.pem` is required as the config map path.
+
* `volumeMounts` creates a `trusted-ca` volume mount.
+
[NOTE]
====
