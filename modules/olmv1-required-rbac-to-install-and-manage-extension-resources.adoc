...
You must inspect the manifests included in the bundle image of a cluster extension to assign the necessary permissions. The service account requires the enough role-based access controls (RBAC) to create and manage the following resources.
...
Custom resource definitions (CRDs):: You must define RBAC so the installation service account can create and manage the CRDs for the extension. Also, you must grant the service account for the controller of the extension the RBAC to manage its CRDs.

Deployments:: You must define RBAC for the installation service account to create and manage the deployments need by the extension controller, such as services and config maps.
...
Roles and role bindings:: You must define RBAC for any roles or role bindings defined in the CSV. The installation service account needs permission create and manage those roles and role bindings.
