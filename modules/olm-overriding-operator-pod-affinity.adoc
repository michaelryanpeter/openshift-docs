...
#...
----
The `nodeAffinity` field defines a node affinity that requires the Operator's pod to be scheduled on a node named `ip-10-0-163-94.us-west-2.compute.internal`.
...
#...
----
The `nodeAffinity` field defines a node affinity that requires the Operator's pod to be scheduled on a node with the `kubernetes.io/arch=arm64` and `kubernetes.io/os=linux` labels.
endif::pod[]
...
#...
----
The `podAffinity` field defines a pod affinity that places the Operator's pod on a node that has pods with the `app=test` label.
...
#...
----
The `podAntiAffinity` field defines a pod anti-affinity that prevents the Operator's pod from being scheduled on a node that has pods with the `cpu=high` label.
endif::node[]
