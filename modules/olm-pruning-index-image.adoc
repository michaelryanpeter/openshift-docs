...
----
$ opm index prune \
    -f {index-image-pullspec} \
    -p {package1},{package2},{package3} \
    [-i {registry-image}] \
    -t <target_registry>:<port>/<namespace>/{index-image}
----
where:

`-f {index-image-pullspec}`:: Specifies the index to prune.
`-p {package1},{package2},{package3}`:: Specifies a comma-separated list of packages to keep.
`-i {registry-image}`:: Specifies the Operator Registry base image with the tag that matches the target {product-title} cluster major and minor version. Required only for {ibm-power-name} and {ibm-z-name} images.
`-t <target_registry>:<port>/<namespace>/{index-image}`:: Specifies a custom tag for new index image being built.
