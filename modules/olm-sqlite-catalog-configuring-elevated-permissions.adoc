...
`security.openshift.io/scc.podSecurityLabelSync`:: Specifies that pod security label synchronization is turned off by setting the value to `false`.
`pod-security.kubernetes.io/enforce`:: Specifies the pod security admission label. Set the label to `baseline` or `privileged`. Use the `baseline` pod security profile unless other workloads in the namespace require a `privileged` profile.
