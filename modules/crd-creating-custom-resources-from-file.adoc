...
where:
+
<1> `apiVersion`:: Specifies the group name and API version (name/version) from the CRD.
<2> `kind`:: Specifies the type in the CRD.
<3> `metadata.name`:: Specifies a name for the object.
<4> `metadata.finalizers`:: Specifies the link:https://kubernetes.io/docs/tasks/access-kubernetes-api/extend-api-custom-resource-definitions/#finalizers[finalizers] for the object, if any. Finalizers allow controllers to implement conditions that must be completed before the object can be deleted.
<5> `spec`:: Specifies conditions specific to the type of object.
