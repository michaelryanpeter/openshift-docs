...
`--bundles <registry>/<namespace>/<bundle_image_name>:<tag>`:: Specifies a comma-separated list of bundle images to add to the index.
`--tag <registry>/<namespace>/<index_image_name>:<tag>`:: Specifies the image tag that you want the index image to have.
`--binary-image <registry_base_image>`:: Specifies an optional alternative registry base image to use for serving the catalog.
