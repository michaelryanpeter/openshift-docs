...
----
+
* `Install Modes` indicates which install modes are supported.
+
* `Name` under a channel's entries shows example channel names.
+
* `Default Channel` shows the channel selected by default if one is not specified.
====
+
[TIP]
====
...
`env`:: Specifies a list of environment variables that must exist in all containers in the pod created by OLM.
`envFrom`:: Specifies a list of sources to populate environment variables in the container.
`volumes`:: Specifies a list of volumes that must exist on the pod created by OLM.
`volumeMounts`:: Specifies a list of volume mounts that must exist in all containers in the pod created by OLM. If a `volumeMount` references a `volume` that does not exist, OLM fails to deploy the Operator.
`tolerations`:: Specifies a list of tolerations for the pod created by OLM.
`resources`:: Specifies resource constraints for all the containers in the pod created by OLM.
`nodeSelector`:: Specifies a `NodeSelector` object for the pod created by OLM.
====
+
.Example `Subscription` object with a specific starting Operator version
[%collapsible]
...
spec:
  config:
    ...
    - name: ROLEARN
      value: "<role_arn>"
----
The `<role_arn>` variable specifies the role ARN details.
====
+
