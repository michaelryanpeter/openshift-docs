...
`metadata.namespace`:: Specifies the `openshift-operators` namespace for default `AllNamespaces` install mode usage. Alternatively, you can specify a custom global namespace, if you have created one. For `SingleNamespace` install mode usage, specify the relevant single namespace.
...
----
+
**  Set the `spec.installPlanApproval` parameter to `Manual` in case your specified version is superseded by a later version in the catalog. This plan prevents an automatic upgrade to a later version and requires manual approval before the starting CSV can complete the installation.
**  Set the `spec.startingCSV` parameter to define a specific version of an Operator CSV.
+
...
`<audience_url>`:: Specifies the `AUDIENCE` value created in {gcp-short} by the administrator when they set up {gcp-wid-short}. The value must be a preformatted URL in the following format:
+
[source,text]
----
