...
----
ifndef::openshift-dedicated,openshift-rosa,openshift-rosa-hcp[]
The `type: Upgradeable` field in the `overrides` section allows the cluster administrator to change the upgrade readiness to `True`.
endif::openshift-dedicated,openshift-rosa,openshift-rosa-hcp[]
ifdef::openshift-dedicated,openshift-rosa,openshift-rosa-hcp[]
The `type: Upgradeable` field in the `overrides` section allows the `dedicated-admin` user to change the upgrade readiness to `True`.
endif::openshift-dedicated,openshift-rosa,openshift-rosa-hcp[]
