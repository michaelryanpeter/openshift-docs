...
spec:
  ...
  source:
    image:
      ...
      ref: registry.redhat.io/redhat/community-operator-index:vBranch Build <4>
    type: Image
----
...
<3> Specify the interval in minutes for polling the remote registry for newer image digests. The default value is `10`. To disable polling, set a zero value.
<4> Specify the catalog's image in the `spec.source.image.ref` field.
