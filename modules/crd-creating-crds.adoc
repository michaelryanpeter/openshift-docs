...
where:
+
<1> `apiextensions.k8s.io/v1`:: Specifies the API version.
<2> `metadata.name`:: Specifies a name for the definition. This must be in the `<plural-name>.<group>` format using the values from the `spec.group` and `spec.names.plural` fields.
<3> `spec.group`:: Specifies a group name for the API. An API group is a collection of objects that are logically related. For example, all batch objects like `Job` or `ScheduledJob` could be in the batch API group (such as `batch.api.example.com`). A good practice is to use a fully-qualified-domain name (FQDN) of your organization.
<4> `spec.versions.name`:: Specifies a version name to be used in the URL. Each API group can exist in multiple versions, for example `v1alpha`, `v1beta`, `v1`.
<5> `spec.scope`:: Specifies whether the custom objects are available to a project (`Namespaced`) or all projects in the cluster (`Cluster`).
<6> `spec.names.plural`:: Specifies the plural name to use in the URL. The `plural` field is the same as a resource in an API URL.
<7> `spec.names.singular`:: Specifies a singular name to use as an alias on the CLI and for display.
<8> `spec.names.kind`:: Specifies the kind of objects that can be created. The type can be in CamelCase.
<9> `spec.names.shortNames`:: Specifies a shorter string to match your resource on the CLI.
+
[NOTE]
====
